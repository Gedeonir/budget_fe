        ...
        <div className='relative bg-primary2 shadow-lg rounded-lg lg:w-2/4 w-full lg:px-4 px-2 py-4'>
            ...
            <form className='text-text_primary'>
                <div className='w-full mb-2'>
                    ...
                    <select className='border w-full px-4 py-2 text-text_primary rounded-lg border-text_primary border-opacity-40' required>
                ...
                <div className='w-full mb-2'>
                    ...
                    <input type="text" name='memberName' className="text-text_secondary rounded-lg outline-primary block w-full px-4 py-1 border border-text_primary border-opacity-40 placeholder-text_primary" placeholder="Member Name" required/>
                </div>

                <div className='w-full mb-2'>
                    <label>Member Position</label>
                    <select className='border w-full px-4 py-2 text-text_primary rounded-lg border-text_primary border-opacity-40' required>
                        <option value={"Dr"}>Minister</option>
                        <option value={"Hon"}>State Minister</option>
                        <option value={"Amb"}>Finance Director</option>
                    </select>
                </div>
                ...
                <div className='w-full'>
                    ...
                    <input type="text" name='Ministry/Institution' className="text-text_secondary rounded-lg outline-primary block w-full px-4 py-1 border border-text_primary border-opacity-40 placeholder-text_primary cursor-pointer" placeholder="Minister/Director" required readOnly onClick={()=>setShowDirector(!showDirector)}/>
                    {showDirector && (
                ...
                <div className='flex justify-between gap-4'>
                    <button type='reset' size="sm" className=' my-4 text-text_primary w-full border-2 border-text_primary border-opacity-40 font-bold p-2' onClick={()=>props.setAddOfficialsModal(false)}>Cancel</button>
                    <button type='submit' size='sm' className={`my-4 bg-secondary text-center text-primary font-bold p-2 w-full ${loading? 'cursor-not-allowed ':'cursor-pointer'}`} disabled={loading? true : false}>
                        {loading?<p className="flex justify-center gap-2"><AiOutlineLoading3Quarters size={20} className="animate-spin h-5 w-5"/><span> Saving institution</span></p>:'Save income'}
                    </button>
                </div>
            </form>
        </div>
