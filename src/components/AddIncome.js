...
const AddIncome = ({ id,setSuccess, setOpenModal, setExpenses, total, totalincome, expenses, incomes, categories, userData, setRevenues, categorytype }) => {
    ...
    function recalculatePercentages(expenses) {
        ...
        const totalAmount = expenses.reduce((total, obj) => total + parseInt(obj.amountToCollect, 10), 0);
        expenses.forEach(obj => {
            obj.percentage = ((obj.amountToCollect / totalAmount) * 100).toFixed(2);
        });
    ...
    const handleAddIncome = async () => {
        ...
        if (!incomeData.income || !incomeData.amountToCollect) {
            ...
            try {
                const sum = allIncomes.reduce((sum, item) => sum + parseInt(item.amountToCollect, 10), 0)
                const amount = sum + parseInt(incomeData.amountToCollect, 10);
                ...
                allIncomes.push(values);
                const response = await axios.patch(`${process.env.BACKEND_URL}/budget/one/${id}/addIncome`, {
                    revenues: recalculatePercentages(allIncomes),
                },
                    {
                        headers: {
    ...
    const expenseItems = incomes?.map(obj => obj.income);
    ...
    const myFilteredCategories = () => {
        return categories?.resp?.data?.filter((item) => item?.institution?.institutionName?.toLowerCase().includes(userData?.getProfile?.institution?.institutionName?.toLowerCase())
            && item.type.toLowerCase().includes("income"))
    }
